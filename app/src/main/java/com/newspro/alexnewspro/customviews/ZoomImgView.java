...
/**
 * 手势缩放图片控件 网上成熟控件，选自：https://github.com/boycy815/PinchImageView
 * 由于自己的缩放控件暂时不适配imageloader（由于延迟可能会造成图片错位，导致浏览失效）
 * 并且暂时没有写入与viewpager的事件拦截机制，也没有大图分块加载的处理，这些问题可能会造成不必要的麻烦， 所以使用这个控件
 * <p>
 * 我增加的： 1.把框架中腹泻viewPager来处理时间拦截的方法放到这里，这样就不用自定义一个viewPager了
 * 2.增加缩小到最小，一般在viewpager的pagechangelistener中判断state，为1的时候执行
 *
 * @author Alex
 */
public class ZoomImgView extends ImageView implements OnPageChangeListener {
    ...
    public interface OuterMatrixChangedListener {
         ...
         */
        void onOuterMatrixChanged(ZoomImgView pinchImageView);
    }
     ...
     * 在onTouchEvent末尾被执行.
     */
    private GestureDetector mGestureDetector = new GestureDetector(ZoomImgView.this.getContext(),
            new GestureDetector.SimpleOnGestureListener() {
                ...
                public void onLongPress(MotionEvent e) {
                    ...
                    if (mOnLongClickListener != null) {
                        mOnLongClickListener.onLongClick(ZoomImgView.this);
                    }
                }
                ...
                public boolean onSingleTapConfirmed(MotionEvent e) {
                    ...
                    if (mOnClickListener != null) {
                        mOnClickListener.onClick(ZoomImgView.this);
                    }
                    return true;
                }
            });
        ...
        if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
            ...
            if (!(mScaleAnimator != null && mScaleAnimator.isRunning())) {
                ...
                if (mPinchMode == PINCH_MODE_SCROLL) {
                    ...
                    // 记录新的移动点
                    mLastMovePoint.set(event.getX(), event.getY());
                    // 如果在放大状态的时候移动则判断是否到边界，如果没到边界不允许父控件拦截
                    if (isDisallowDrag(event.getX() - mLastPoint.x)) {
    ...
    /**
     * 缩放到最小 一般在viewpager的pagechangelistener中判断state，为1的时候执行
     **/
    public void toMin() {
        if (!isReady()) {
            return;
        ...
        Matrix animEnd = MathUtils.matrixTake(mOuterMatrix);
        // 计算还需缩放的倍数
        animEnd.postScale(nextScale / currentScale, nextScale / currentScale, getWidth() / 2, getHeight() / 2);
        // 将放大点移动到控件中心
        animEnd.postTranslate(displayWidth / 2f - getWidth() / 2, displayHeight / 2f - getHeight() / 2);
        // 得到放大之后的图片方框
        Matrix testMatrix = MathUtils.matrixTake(innerMatrix);
        testMatrix.postConcat(animEnd);
        RectF testBound = MathUtils.rectFTake(0, 0, getDrawable().getIntrinsicWidth(),
